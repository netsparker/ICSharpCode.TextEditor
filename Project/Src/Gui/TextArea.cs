...
namespace ICSharpCode.TextEditor
{
	...
	public class TextArea : Control
	{
		...
		private readonly List<BracketHighlightingSheme> _bracketshemes = new List<BracketHighlightingSheme>();

		private bool _disposed;
		private bool _hiddenMouseCursor;
		...
		private AbstractMargin _updateMargin;
...
//			SetStyle(ControlStyles.UserPaint, true);
		...
		[Browsable(false)]
		public IList<AbstractMargin> LeftMargins => _leftMargins.AsReadOnly();
		...
		public int MaxVScrollValue
			=>
				(Document.GetVisibleLine(Document.TotalNumberOfLines - 1) + 1 + TextView.VisibleLineCount * 2 / 3) *
				TextView.FontHeight;
		...
		private void SearchMatchingBracket(object sender, EventArgs e)
		{
			...
			if (TextView.Highlight != null && TextView.Highlight.OpenBrace.Y >= 0 &&
			    TextView.Highlight.OpenBrace.Y < Document.TotalNumberOfLines)
		...
		public Highlight FindMatchingBracketHighlight()
		{
			if (Caret.Offset == 0)
				return null;
			foreach (var bracketsheme in _bracketshemes)
			{
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			{
				var realmousepos = TextView.GetLogicalPosition(e.X - TextView.DrawingPosition.X,
					e.Y - TextView.DrawingPosition.Y);
