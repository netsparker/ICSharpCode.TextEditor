		...
		public override void Execute(TextArea textArea)
		{
			...
			if (textArea.SelectionManager.HasSomethingSelected)
			{
				...
				foreach (var selection in textArea.SelectionManager.SelectionCollection)
				{
					...
					textArea.Document.UpdateQueue.Clear();
					textArea.Document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.LinesBetween, _firstLine, _lastLine));
					textArea.EndUpdate();
				}
				textArea.Document.CommitUpdate();
				textArea.AutoClearSelection = false;
	...
	public class ToggleBlockComment : AbstractEditAction
	{
		...
		public override void Execute(TextArea textArea)
		{
			...
			if (textArea.SelectionManager.HasSomethingSelected)
			{
				...
				selectionEndOffset =
					textArea.SelectionManager.SelectionCollection[textArea.SelectionManager.SelectionCollection.Count - 1].EndOffset;
			}
			else
			{
			...
			var commentRegion = FindSelectedCommentRegion(textArea.Document, commentStart, commentEnd,
		...
		public static BlockCommentRegion FindSelectedCommentRegion(IDocument document, string commentStart, string commentEnd,
			...
			if (commentStartOffset == -1)
			{
				...
				if (commentStartOffset >= 0)
				{
					...
					commentEndBeforeStartOffset = text.IndexOf(commentEnd, commentStartOffset,
						selectionStartOffset - commentStartOffset);
...
//						textArea.Document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.PositionToLineEnd, new TextLocation(textArea.Caret.Offset - textArea.Document.GetLineSegment(curLineNr).Offset, curLineNr)));
	...
	public class MovePageDown : AbstractEditAction
	{
		...
		public override void Execute(TextArea textArea)
		{
			...
			var requestedLineNumber =
				Math.Min(textArea.Document.GetNextVisibleLineAbove(curLineNr, textArea.TextView.VisibleLineCount),
					textArea.Document.TotalNumberOfLines - 1);
	...
	public class MovePageUp : AbstractEditAction
	{
		...
		public override void Execute(TextArea textArea)
		{
			var curLineNr = textArea.Caret.Line;
			var requestedLineNumber =
				Math.Max(textArea.Document.GetNextVisibleLineBelow(curLineNr, textArea.TextView.VisibleLineCount), 0);
	...
	public class WordBackspace : AbstractEditAction
	{
		...
		public override void Execute(TextArea textArea)
		{
			...
			{
				var prevWordStart = TextUtilities.FindPrevWordStart(textArea.Document,
