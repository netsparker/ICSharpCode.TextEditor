		...
		public bool SelectionIsReadonly
		{
			get
			{
				...
				foreach (var sel in selectionCollection)
					if (SelectionIsReadOnly(_document, sel))
						return true;
				return false;
			}
		}
...
//			autoClearSelection = false;
			if (selection != null)
			{
				...
				_document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.LinesBetween, selection.StartPosition.Y,
...
//			}
		...
		public ColumnRange GetSelectionAtLine(int lineNumber)
		{
			foreach (var selection in selectionCollection)
			{
				...
				{
					var line = _document.GetLineSegment(startLine);
