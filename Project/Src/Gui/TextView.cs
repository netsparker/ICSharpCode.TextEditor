...
namespace ICSharpCode.TextEditor
{
	...
	public class TextView : AbstractMargin, IDisposable
	{
		...
		{
			get { return textArea.Document.GetFirstLogicalLine(textArea.VirtualTop.Y / FontHeight); }
			set
			{
		...
		public void OptionsChanged()
		{
			...
			SpaceWidth = Math.Max(GetWidth(' ', _lastFont), 1);
			// tab should have the width of 4*'x'
			WideSpaceWidth = Math.Max(SpaceWidth, GetWidth('x', _lastFont));
		}
...
//				bgColorBrush.Dispose();
			...
			if (TextEditorProperties.EnableFolding)
				while (true)
				{
					var starts = textArea.Document.FoldingManager.GetFoldedFoldingsWithStartAfterColumn(lineNumber,
						column - 1);
					if (starts == null || starts.Count <= 0)
					{
						if (lineNumber < textArea.Document.TotalNumberOfLines)
							physicalXPos = PaintLinePart(g, lineNumber, column, textArea.Document.GetLineSegment(lineNumber).Length,
				...
				physicalXPos = PaintLinePart(g, lineNumber, 0, textArea.Document.GetLineSegment(lineNumber).Length, lineRectangle,
					physicalXPos);
			...
			if (lineNumber < textArea.Document.TotalNumberOfLines)
			{
				...
				if (TextEditorProperties.ShowEolMarker)
				{
					...
					physicalXPos += DrawEolMarker(g, eolMarkerColor.Color, selectionBeyondEol ? bgColorBrush : backgroundBrush,
						physicalXPos, lineRectangle.Y);
				}
				else
				{
...
//			bgColorBrush.Dispose();
			...
			bool drawSelected)
		{
			...
			DrawString(g,
				...
				rect.X + 1, rect.Y);
			g.DrawRectangle(BrushRegistry.GetPen(drawSelected ? Color.DarkGray : Color.Gray), rect.X, rect.Y, rect.Width,
				rect.Height);
			...
			int physicalXPos)
		{
			...
			for (var wordIdx = 0; wordIdx < currentLine.Words.Count; wordIdx++)
			{
				...
				var wordBackBrush = GetMarkerBrushAt(currentLine.Offset + currentWordOffset, currentWord.Length, ref wordForeColor,
					out markers);
				...
				if (Highlight != null)
					if (Highlight.OpenBrace.Y == lineNumber && Highlight.OpenBrace.X == currentWordOffset ||
						...
						DrawBracketHighlight(g,
							new Rectangle((int) wordRectangle.X, lineRectangle.Y, (int) wordRectangle.Width - 1, lineRectangle.Height - 1));
		...
		private struct WordFontPair
		{
			...
			public override bool Equals(object obj)
			{
				...
				if (!_word.Equals(myWordFontPair._word)) return false;
				return _font.Equals(myWordFontPair._font);
			}

			public override int GetHashCode()
			{
				return _word.GetHashCode() ^ _font.GetHashCode();
			}
		}
		...
		private const TextFormatFlags TextFormatFlags =
			System.Windows.Forms.TextFormatFlags.NoPadding | System.Windows.Forms.TextFormatFlags.NoPrefix |
		...
		private readonly Dictionary<Font, Dictionary<char, int>> _fontBoundCharWidth =
		...
		internal TextLocation GetLogicalColumn(int lineNumber, int visualPosX, out FoldMarker inFoldMarker)
		{
			...
			using (var g = textArea.CreateGraphics())
			{
				...
				while (true)
				{
					...
					var newPosX = posX + 1 +
					              MeasureStringWidth(g, nextFolding.FoldText, TextEditorProperties.FontContainer.RegularFont);
