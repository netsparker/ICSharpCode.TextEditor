...
namespace ICSharpCode.TextEditor
{
	...
	public class GutterMargin : AbstractMargin, IDisposable
	{
		...
		private readonly StringFormat _numberStringFormat = (StringFormat) StringFormat.GenericTypographic.Clone();

		static GutterMargin()
		{
			var cursorStream =
				Assembly.GetCallingAssembly().GetManifestResourceStream("ICSharpCode.TextEditor.Resources.RightArrow.cur");
		...
		public GutterMargin(TextArea textArea) : base(textArea)
		{
			...
			_numberStringFormat.FormatFlags = StringFormatFlags.MeasureTrailingSpaces | StringFormatFlags.FitBlackBox |
		...
		public override void Paint(Graphics g, Rectangle rect)
		{
			...
			for (var y = 0; y < (DrawingPosition.Height + textArea.TextView.VisibleLineDrawingRemainder) / fontHeight + 1; ++y)
			{
				...
				if (rect.IntersectsWith(backgroundRectangle))
				{
					...
					var curLine =
						textArea.Document.GetFirstLogicalLine(textArea.Document.GetVisibleLine(textArea.TextView.FirstVisibleLine) + y);
		...
		public override void HandleMouseDown(Point mousepos, MouseButtons mouseButtons)
		{
			...
			if (realline >= 0 && realline < textArea.Document.TotalNumberOfLines)
				if ((Control.ModifierKeys & Keys.Shift) != 0)
				{
					if (!textArea.SelectionManager.HasSomethingSelected && realline != textArea.Caret.Position.Y)
					{
						if (realline >= textArea.Caret.Position.Y)
						{
							...
							if (realline < textArea.Document.TotalNumberOfLines - 1)
							{
								textArea.SelectionManager.SetSelection(new DefaultSelection(textArea.Document, selectionStartPos,
									new TextLocation(0, realline + 1)));
								textArea.Caret.Position = new TextLocation(0, realline + 1);
							}
							else
							{
								textArea.SelectionManager.SetSelection(new DefaultSelection(textArea.Document, selectionStartPos,
									new TextLocation(textArea.Document.GetLineSegment(realline).Length + 1, realline)));
						...
						else
						{
							...
							// whole line selection - start of line to start of next line
							textArea.SelectionManager.SetSelection(new DefaultSelection(textArea.Document, selectionStartPos,
								new TextLocation(selectionStartPos.X, selectionStartPos.Y)));
							textArea.SelectionManager.ExtendSelection(new TextLocation(selectionStartPos.X, selectionStartPos.Y),
					...
					else
					{
						...
						var e = new MouseEventArgs(mouseButtons, 1, mousepos.X, mousepos.Y, 0);
				...
				else
				{
					...
					{
						textArea.SelectionManager.SetSelection(new DefaultSelection(textArea.Document, new TextLocation(0, realline),
							new TextLocation(textArea.Document.GetLineSegment(realline).Length + 1, selectionStartPos.Y)));
						textArea.Caret.Position = new TextLocation(textArea.Document.GetLineSegment(realline).Length + 1,
							selectionStartPos.Y);
					}
				}
		}
